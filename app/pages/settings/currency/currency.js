var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var ionic_angular_1 = require('ionic-angular');
var currency_1 = require('../../../providers/currency/currency');
// services
var CurrencyPage = (function () {
    function CurrencyPage(currencyService) {
        var _this = this;
        this.currencyService = currencyService;
        this.exchangeServices = currencyService.getAvailabeServices();
        Promise.all([
            currencyService.getSelectedService(),
            currencyService.getSelectedCurrency(),
            currencyService.getAvailableCurrencies()
        ]).then(function (selections) {
            _this.selectedExchange = selections[0];
            _this.selectedCurrency = selections[1];
            _this.availableCurrencies = selections[2];
        });
    }
    CurrencyPage.prototype.exchangeChanged = function () {
        this.currencyService.setSelectedService(this.selectedExchange);
    };
    CurrencyPage.prototype.currencyChanged = function () {
        this.currencyService.setSelectedCurrency(this.selectedCurrency);
    };
    CurrencyPage = __decorate([
        ionic_angular_1.Page({
            templateUrl: 'build/pages/settings/currency/currency.html'
        }), 
        __metadata('design:paramtypes', [currency_1.Currency])
    ], CurrencyPage);
    return CurrencyPage;
})();
exports.CurrencyPage = CurrencyPage;
